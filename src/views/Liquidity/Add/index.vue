        ...
        <div class="wrap">
          <div class="flex flex-ac flex-bt">
            ...
            <div v-else class="fs14b currency" @click="selectToken">
              <img
                class="icon-coin"
                :src="require(`@/assets/coin/coin_${token}.png`)"
                alt=""
              />
              <span>{{ token }}</span>
...
export default {
  ...
  methods: {
    ...
    checkCoin(val) {
      if (this.selectType === "token") {
        if (this.token != val) {
          this.token = val;
          this.$router.push(
            `/pool/add/${this.currency.toLowerCase()}/${val.toLowerCase()}`
          );
        }
    ...
    handleMax() {
      if (this.allowance) {
        this.amount = this.formatDecimals(
          Math.min(this.allowance, this.token0BalanceOf)
        );
      } else {
