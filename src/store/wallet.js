...
export default {
  ...
  async initContract({ state, commit, dispatch }, payload = {}) {
    ...
    if (!notFirst) { // networkId首次连接监听事件，只切换账户不需要（会重复监听）
  ...
  contractEvents({ state, commit }) {
    ...
    commit('clearTrades');
    state.contract.getPastEvents('Trade', {
      fromBlock: 0,
      toBlock: 'latest'
    }, (err, event) => {
      if (err) {
        ...
        if (event.length) {
          const _list = event.map(item => {
            ...
            return { amount, direction, price, blockNumber: item.blockNumber, transactionHash: item.transactionHash }
          })
    ...
    state.contract.events.Trade({
      ...
      if (err) {
        ...
        if (event.event === 'Trade') {
          ...
          commit('setTrade', { amount, direction, price, time: block.timestamp * 1000, blockNumber: event.blockNumber, transactionHash: event.transactionHash })
        }
      }
    })
  },
  ...
  async getPosition({ state, commit }) {
    ...
    commit("setPosition", { longAmount: longAmount * 1, longPrice: longPrice * 1, margin: margin * 1, shortAmount: shortAmount * 1, shortPrice: shortPrice * 1 })
    let poolNet
    try {
