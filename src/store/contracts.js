export default {
  ...
  // 开多仓函数
  async openLong({ state }, { priceExp, amount, rechargeAmount }) {
    return await state.contract.methods.openLong(priceExp, amount, rechargeAmount).send({ from: state.coinbase })
  },
  // 开空仓函数
  async openShort({ state }, { priceExp, amount, rechargeAmount }) {
    return await state.contract.methods.openShort(priceExp, amount, rechargeAmount).send({ from: state.coinbase })
  },
  // 保证金充值
