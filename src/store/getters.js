export default {
  ...
  // UsedMargin=(longAmount+shortAmount)*price/leverage;
  UsedMargin(state) {
    if (JSON.stringify(state.position) !== '{}') {
      const { longAmount, shortAmount } = state.position
      const { price, leverage } = state
      return Math.floor(((longAmount + shortAmount) * price) / leverage);
    } else {
      return 0;
    }
  },
  ...
  NetValue(state) {
    if (JSON.stringify(state.position) !== '{}') {
      ...
      const { price } = state
      return (
        longAmount * (price - longPrice) +
        shortAmount * (shortPrice - price) +
        margin * 1
      );
    } else {
      return 0;
  ...
  LiquidationPrice(state) {
    ...
    if (JSON.stringify(state.position) !== '{}') {
      ...
      return (keepMargin - margin + longAmount * longPrice - shortAmount * shortPrice) / (longAmount - shortAmount)
    } else {
      return 0
    }
  },
