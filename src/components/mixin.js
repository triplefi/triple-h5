...
export default {
  computed: {
    ...mapState(['web3', 'coinbase', 'contract', 'contractAddress', 'decimals', 'amountDecimal', 'token0', 'price', 'deadline']),
    ...mapGetters(['marginCoin', 'tradeCoin'])
  },
  methods: {
    ...mapActions(['refreshData']),
    formatDecimals(val) { // 合约换算和精度
      return formatNum(val / Math.pow(10, this.decimals), this.decimals * 1)
    },
    pricePrecision(val) { // 价格换算和精度
    ...
    amountPrecision(val) { // amount换算和精度
